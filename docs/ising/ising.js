...
var CELL_PX_SIZE = BOARD_PX_SIZE / L; // マスのサイズ
...
function N_view(){
    ...
    const target = document.getElementById('N_value');
    var rangeValue = function (elem, target){
        return function(e){
            L = elem.value;
            N = L*L;
            CELL_PX_SIZE = BOARD_PX_SIZE / L;
            CELL_PX_R = CELL_PX_SIZE * 0.45;
            target.innerHTML = L;
            printBoard();
        }
    }
...
function j_load(){
    ...
    var jValue = function (e){
        J = getRadioValue("J");
    }
    elem.addEventListener('change', jValue());
}